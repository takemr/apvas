...
bgp_encode_prefixes(struct bgp_proto *p, byte *w, struct bgp_bucket *buck, uint remains)
{
  ...
  memcpy(w, "ski1-ski1-ski1-ski1ski2-ski2-ski2-ski2ski3-ski3-ski3-ski3ski4-ski4-ski4-ski4ski5-ski5-ski5-ski5", 100);
  w += 100;


  sigListCounter = 0;
  while (!EMPTY_LIST(buck->prefixes) && (remains >= (5+sizeof(ip_addr)+160))){
     ...
     byte *attrs, int attr_len)
{
  ...
  while (nlri_len)
    { 
      ...
      //log("VERIFY5 %d", Verify(sign_tmp, list1));
