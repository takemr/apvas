...
bgp_encode_prefixes(struct bgp_proto *p, byte *w, struct bgp_bucket *buck, uint remains)
{
  ...
  memcpy(w, "ski1-ski1-ski1-ski1ski2-ski2-ski2-ski2ski3-ski3-ski3-ski3ski4-ski4-ski4-ski4", 80);
  w += 80;

  sigListCounter = 0;
  while (!EMPTY_LIST(buck->prefixes) && (remains >= (5+sizeof(ip_addr)+140))){
     ...
     List list1[list_size];
      ...
      char m1[str_size] = "";
      bsprintf(m1, "ski1-ski1-ski1-ski1 %I/%d", px->n.prefix, px->n.pxlen);
     ...
     byte *attrs, int attr_len)
{
  ...
  while (nlri_len)
    {  
      ...
      //log("VERIFY4 %d", Verify(sign_tmp, list1));
